...
The receive-only probe-sender mailbox and required MX/SRS records are documented in
[probe sender mailbox forwarder](deploy/mail-forwarder/README.md).
...
Every `Unknown` result includes an additive `unknownContext` object identifying the stable cause, a human-readable
summary, whether a later attempt may be useful, and the recommended next action. When available it also carries the
SMTP category, failed stage, response/enhanced code, MX host, and retry time. `retryable` describes the evidence; it
does not mean automatic retry is enabled or scheduled. Lifecycle, REST, gRPC, Console, JSON, and CSV projections all
use this same host-independent context.
